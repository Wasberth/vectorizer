...
import sys
import pickle
from utilsCNN import compareColors, parseColor, get_valid_indices, create_from_indices

root = 'dataset/'
input_directory = root+"inputCNN/"
output_directory = root+"outputCNN/"
file_num = 0
vector_num = 14
...
def saveCoords(coord_x, coord_y, class_balancer, matrix, type):
    control_id = np.argsort(class_balancer[type])
    control_bias = 0
    if type == 0:
    ...
    for i in range(used_coord):
        writing = True
        while writing:
            if matrix[(control_id[i + control_bias]*3)] == -1:
                matrix[(control_id[i + control_bias]*3)] = type
                matrix[(control_id[i + control_bias]*3) + 1] = coord_x[i]
                matrix[(control_id[i + control_bias]*3) + 2] = coord_y[i]
                class_balancer[type][control_id[i + control_bias]] += 1
                writing = False
                control_id[i + control_bias] = -1
            else:
                control_bias += 1
    return matrix

def concatMatrixCNN(img_pil, paths, attributes, color_classes, lock):
    valid = True
    width, height = img_pil.size
    input_matrix = np.zeros((width, height, 1))
    output_matrix = np.zeros((vector_num*3))
    ...
    max_x_value = max(x_se)
    max_control = max(x_control)
    if max_control > max_x_value:
        max_x_value = max_control
    max_y_value = max(y_se)
    max_control = max(y_control)
    if max_control > max_y_value:
        max_y_value = max_control
    factor = 1
    ...
    for i in range(len(x_se)):
        x_se[i] = x_se[i]*factor
        y_se[i] = y_se[i]*factor
    for i in range(len(x_control)):
        x_control[i] = x_control[i]*factor
        y_control[i] = y_control[i]*factor
    with lock:
        class_balancer = np.memmap(f'{root}class_balancer.npy', dtype=np.int64, mode='r+', shape=(2,vector_num))
        output_matrix = saveCoords(x_se, y_se, class_balancer, output_matrix, 0)
        output_matrix = saveCoords(x_control, y_control, class_balancer, output_matrix, 1)
        class_balancer.flush()
...
def generateCNNMatrix(idx, idn, input, output, input_shape, output_shape, batch_size, thread_id, lock):
    ...
    for i in range(iterations):
        ...
        for j in range(len(processing_ids)):
            input_matrices[j], output_matrices[j], valid = concatMatrixCNN(imgs[j], paths[j], attributes[j], color_classes[j],  lock)
            if not valid:
...
if __name__ == "__main__":
    ...
    with multiprocessing.Manager() as manager:
        ...
        for file in os.listdir(directory):
            ...
            if regex != None:
                ...
                if coord_num > 11 and coord_num < 15:
        ...
        if create_files:
            ...
            class_balancer = np.memmap(f'{root}class_balancer.npy', dtype=np.int64, mode='w+', shape=(2, vector_num))
            class_balancer[:] = 0

        cursorPrint(available_threads, "Procesando " + str(input_shape[0]) + " matrices")
        pool.starmap(generateCNNMatrix, thread_info)
        pool.close()
        pool.join()

    class_balancer = np.memmap(f'{root}class_balancer.npy', dtype=np.int64, mode='r', shape=(2, vector_num))
    cursorPrint(available_threads+1, f'Hay un máximo de {max(class_balancer[0])} registros en una posición y un mínimo de {min(class_balancer[0])} en los registros para SE, hay un total de {np.sum(class_balancer[0])}')
    cursorPrint(available_threads+2, f'Hay un máximo de {max(class_balancer[1])} registros en una posición y un mínimo de {min(class_balancer[1])} en los registros para control, hay un total de {np.sum(class_balancer[1])}')
    ...
    if len(indices) < input_shape[0]:
        ...
        output_shape = (len(indices), vector_num*3)
        indices = get_valid_indices(output_directory+f"{dataset_sufix}_filtered.npy", output_shape, 1, np.float64)
    else:
        indices = get_valid_indices(output_directory+f"{dataset_sufix}.npy", output_shape, 1, np.float64)
    if len(indices) < input_shape[0]:
        create_from_indices(input_directory+f"{dataset_sufix}.npy", input_shape, np.uint8, indices)
        create_from_indices(output_directory+f"{dataset_sufix}.npy", output_shape, np.float64, indices)
        input_shape = (len(indices), input_shape[1], input_shape[2], input_shape[3])
