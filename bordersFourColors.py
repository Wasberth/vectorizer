...
def mode_filter_grayscale(image, kernel_size=3):
    """
    Aplica un filtro basado en la moda en una imagen en escala de grises.

    :param image: ndarray, imagen de entrada en escala de grises.
    :param kernel_size: int, tamaño del kernel (debe ser impar).
    :return: ndarray, imagen filtrada con el filtro de moda.
    """
    ...
    # Recorrer cada píxel en la imagen original
    ...
    for i in range(image.shape[0]):
        for j in range(image.shape[1]):
            ...
            # Calcular la moda
            mode_calculated = mode(neighborhood, axis=None).mode
            if type(mode_calculated) is list or type(mode_calculated) is tuple:
                mode_calculated = mode_calculated[0]
            filtered_image[i, j] = mode_calculated

            percent += 1
            print(f"{np.round(10000 * (percent / pixels))/100}% ({percent} pixeles / {pixels})", end='\r')
    print(f"100%", end='\n')
...
if __name__ == "__main__":
    ...
    # Reducción de ruido
    smoothed_image = mode_filter_grayscale(labeled_image)
    print("ruido")

    # Mostrar y guardar resultados
    cv2.imshow("Segmentación coloreada", colored_image)
    cv2.waitKey(0)
